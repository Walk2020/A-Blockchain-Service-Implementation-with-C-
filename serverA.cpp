...
#define PORT_NUMBER_of_SERVER_A 21128
...
int sock_descriptor_serverA;

int main(){
    string file_name = "block1.txt"; // serverA's data file 
    int number_of_records = 0; // number of records in the data file, initialize with 0;


    // bootup message
    cout << "The ServerA is up and running using UDP on port" << PORT_NUMBER_of_SERVER_A << "." << endl;

    //initialize the information of serverA
    memset(&serverA_info, 0, sizeof(serverA_info));
    serverA_info.sin_family = AF_INET; //set IPV4
    serverA_info.sin_port = htons(PORT_NUMBER_of_SERVER_A); //set port#
    ...
    main_server_info.sin_addr.s_addr = inet_addr(IP_ADDRESS); // serverA's IP address
    ...
    while(true){
        ...
        if(size_of_vector == 1 && splitted_command[0] == "read_all"){
            ...
            send_to_main_server(result.c_str());
            cout << "The serverA finished writing a new reord from the Main Server." << endl;
        }
...
void set_UDP_socket(){
    sock_descriptor_serverA = socket(serverA_info.sin_family, SOCK_DGRAM, 0);
    if(sock_descriptor_serverA == -1){
...
void bind_socket(){
    if (bind(sock_descriptor_serverA, (struct sockaddr *) &serverA_info, sizeof(serverA_info)) == -1) {
...
void receive_from_main_server(){
    ...
    int bytes_recv = recvfrom(sock_descriptor_serverA, buffer_for_main_server, BUFFER_SIZE - 1, 0, (struct sockaddr *) &main_server_info, &main_server_info_size);
    if(bytes_recv == -1){
        perror("ServerA: fail to receive data from main server");
        close(sock_descriptor_serverA);
        exit(1);
    }
...
void send_to_main_server(const char* message){
    ...
    socklen_t main_server_info_size = sizeof(main_server_info);
    int bytes_sent = sendto(sock_descriptor_serverA, message, len, 0, (struct sockaddr *) &main_server_info, main_server_info_size);
    if(bytes_sent == -1){
        perror("ServerA: fail to send data to main server");
        close(sock_descriptor_serverA);
        exit(1);
    }
