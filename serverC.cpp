...
#define PORT_NUMBER_of_SERVER_C 23128
...
int sock_descriptor_serverC;

int main(){
    string file_name = "block3.txt"; // serverC's data file 
    int number_of_records = 0; // number of records in the data file, initialize with 0;


    // bootup message
    cout << "The ServerC is up and running using UDP on port" << PORT_NUMBER_of_SERVER_C << "." << endl;

    //initialize the information of serverC
    memset(&serverC_info, 0, sizeof(serverC_info));
    serverC_info.sin_family = AF_INET; //set IPV4
    serverC_info.sin_port = htons(PORT_NUMBER_of_SERVER_C); //set port#
    ...
    main_server_info.sin_addr.s_addr = inet_addr(IP_ADDRESS); // set main server's IP address

    //create a UDP socket
    set_UDP_socket();

    //bind the socket with serverA's IP address and the port number of it
    bind_socket();
    ...
    while(true){
        ...
        if(size_of_vector == 1 && splitted_command[0] == "read_all"){
            ...
            send_to_main_server(result.c_str());
            cout << "The serverC finished writing a new reord from the Main Server." << endl;
        }
...
void set_UDP_socket(){
    sock_descriptor_serverC = socket(serverC_info.sin_family, SOCK_DGRAM, 0);
    if(sock_descriptor_serverC == -1){
        perror("clientC: socket");
        exit(1);
    }
}

void bind_socket(){
    if (bind(sock_descriptor_serverC, (struct sockaddr *) &serverC_info, sizeof(serverC_info)) == -1) {
        perror("ServerA: fail to bind the socket to port 21128");
        exit(1);
    }
}
...
void receive_from_main_server(){
    ...
    int bytes_recv = recvfrom(sock_descriptor_serverC, buffer_for_main_server, BUFFER_SIZE - 1, 0, (struct sockaddr *) &main_server_info, &main_server_info_size);
    if(bytes_recv == -1){
        perror("ServerC: fail to receive data from main server");
        close(sock_descriptor_serverC);
        exit(1);
    }
...
void send_to_main_server(const char* message){
    ...
    socklen_t main_server_info_size = sizeof(main_server_info);
    int bytes_sent = sendto(sock_descriptor_serverC, message, len, 0, (struct sockaddr *) &main_server_info, main_server_info_size);
    if(bytes_sent == -1){
        perror("ServerC: fail to send data to main server");
        close(sock_descriptor_serverC);
        exit(1);
    }
